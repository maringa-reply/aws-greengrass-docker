...
**1.1** Download and decompress the `aws-greengrass-docker-main` package. [Dowload from Git.](https://github.com/aws-greengrass/aws-greengrass-docker)
  
**1.2** In a terminal, run the following commands in the location where you decompressed the `aws-greengrass-docker-main` package.
```  
cd ~/Downloads/aws-greengrass-docker-main
sudo docker build -t "x86_64/aws-iot-greengrass:main" ./
```

**1.2.1** If you want to cross-build for a different platform, aarch64 for example.
```
cd ~/Downloads/aws-greengrass-docker-main
sudo docker build --platform <platform> -t "aws-iot-greengrasss:main" ./
```
 ...
 * **Note**: If you want to provision the device upon startup for cloud deployments, you will need to add the following lines to your docker-compose file.
...
x86-64/aws-iot-greengrass           main	3f152d6707c8        17 seconds ago      695MB
```  
...
x86_64/aws-iot-greengrass:main
```
...
**2.1.3**  If you have `docker-compose` installed, you can run the following commands instead:  
```  
cd ~/Downloads/aws-greengrass-docker-main
...
This will start the container with an interactive shell (sh in this example). If the container is already running, you can also use:

```bash
docker compose exec myservice sh
```

### Tutorial: Develop and test a component on your device
[Link to tutorial](https://docs.aws.amazon.com/greengrass/v2/developerguide/create-first-component.html)
